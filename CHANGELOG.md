...
1. **Distribution is now stable.** `gem install ui_guardrails` and `gem "ui_guardrails", "~> 1.0"` both work without git-tag gymnastics. RubyGems Trusted Publishing handles the release pipeline (see `doc/PUBLISHING.md`).
...
The gem ships as **`ui_guardrails`** on RubyGems, not `guardrails`. Why: the unqualified name was rejected by RubyGems' [similarity-rule typo-squatting prevention](https://blog.rubygems.org/2023/12/14/typo-squatting-prevention.html) against [`guard-rails`](https://rubygems.org/gems/guard-rails) (an unrelated Guard plugin that restarts Rails on file changes, 5.4M downloads). The two projects have nothing to do with each other but `guardrails` is one hyphen away from `guard-rails`, so the algorithm flagged it.

For users:

- **Gemfile:** `gem "ui_guardrails", "~> 1.0"`
- **In code:** `require "guardrails"` (unchanged — module + entry point keep the project name; only the package name carries the `ui_` prefix)
- **Rake tasks:** `rake guardrails:audit` (unchanged)
- A `lib/ui_guardrails.rb` shim makes default Bundler auto-require work without users having to know about the rename.
...
- README install snippet now uses `gem "ui_guardrails", "~> 1.0"`.
